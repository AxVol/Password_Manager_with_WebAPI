...
public class Kuznechik : IKuznechik
{
    ...
    public byte[] Encrypt(byte[] data) 
    {
        ...
        for (int i = 0; i < 9; i++) 
        {
            LSX(ref temp, ref subKeys[i], ref block);
            Array.Copy(temp, block, BLOCK_SIZE);
        }

        X(ref block, ref subKeys[9]);
	...
	public byte[] Decrypt(byte[] data) 
	{
		...
		for (int i = 9; i >= 1; i--)
		{
			X(ref block, ref subKeys[i]);
			reverseL(ref block);
			reverseS(ref block);
		}

		X(ref block, ref subKeys[0]);
	...
	public void SetKey(byte[] key) 
	{
        ...
        {
            subKeys[0][i] = x[i] = key[i];
            subKeys[1][i] = y[i] = key[i + 16];
        }
        ...
        for (int k = 1; k < 5; k++) 
        {
            ...
            Array.Copy(x, subKeys[2 * k], SUB_LENGTH);
            Array.Copy(y, subKeys[2 * k + 1], SUB_LENGTH);
        }
	}
